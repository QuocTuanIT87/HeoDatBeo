...
import { PiggyBank, Edit2, Info, History as HistoryIcon } from "lucide-react-native";
import { storage } from "../store/storage";
import { Transaction, UserProfile, CategoryBudget, SavingHistoryItem } from "../types";
...
const SavingScreen = () => {
  ...
  const loadData = async () => {
    ...
    if (p.savingYear && p.savingYear < currentYear) {
      ...
      Alert.alert("Năm mới đã đến! 🧧", `Hệ thống đã lưu lại kết quả tiết kiệm năm ${p.savingYear} vào lịch sử. Hãy đặt mục tiêu mới cho năm ${currentYear} nhé!`);
    } else {
      setProfile(p);
      if (!p.savingYear) {
    ...
    const withdrawals = transactions.filter(t => t.category === "Rút tiết kiệm");
  ...
  const handleSaveTarget = async () => {
    ...
    if (profile) {
      ...
      Alert.alert("Thành công", `Đã thiết lập mục tiêu tiết kiệm cho năm ${currentYear}.`);
    }
  };

  const executeTransaction = async () => {
    if (!profile?.savingTarget) {
      Alert.alert("Chưa có mục tiêu", `Vui lòng đặt mục tiêu tiết kiệm cho năm ${currentYear} trước.`);
    ...
    if (type === "deposit" && amount > unallocated) {
      Alert.alert("Lỗi", `Số dư chưa phân bổ (${formatCurrency(unallocated)} đ) không đủ.`);
      return;
    }
    ...
    if (type === "withdraw") {
      ...
      if (cooldownRemainingDays > 0) {
        Alert.alert("Chưa thể rút", `Vui lòng đợi thêm ${cooldownRemainingDays} ngày nữa.`);
        return;
      }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerTitleRow}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate("SavingHistory")} style={styles.historyBtn}>
        ...
        <View style={styles.balancesContainer}>
          ...
          <View style={[styles.balanceRow, { marginBottom: 0 }]}>
            ...
            <Text style={[styles.balanceAmount, { color: "#fcd34d" }]}>{formatCurrency(savingBalance)} đ</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent}>
        <View style={styles.targetCard}>
          <View style={styles.targetHeader}>
            ...
            {profile?.savingHistory && profile.savingHistory.length > 0 && (
              <TouchableOpacity onPress={() => navigation.navigate("SavingHistory")} style={styles.viewHistoryBtn}>
          ...
          {profile?.savingTarget ? (
            <View>
              ...
              <View style={styles.progressContainer}>
                <View style={[styles.progressBar, { width: `${Math.min(100, (savingBalance / profile.savingTarget) * 100)}%` }]} />
              </View>
              <Text style={styles.progressText}>Đạt được {((savingBalance / profile.savingTarget) * 100).toFixed(1)}% mục tiêu</Text>
              {!canEditTarget() && !isEditingTarget && <Text style={styles.cooldownText}>* Có thể sửa lại sau {getDaysUntilEdit()} ngày</Text>}
            </View>
          ) : (
            <View style={styles.targetEditRow}>
              <TextInput style={styles.targetInput} keyboardType="numeric" value={targetInput} onChangeText={(text) => setTargetInput(formatMoneyInput(text))} placeholder="Nhập mục tiêu năm mới..." />
              <TouchableOpacity style={styles.saveTargetBtn} onPress={handleSaveTarget}>
        ...
        <View style={styles.tabs}>
          <TouchableOpacity style={[styles.tab, type === "deposit" && styles.tabActiveDeposit]} onPress={() => setType("deposit")}>
            <Text style={[styles.tabText, type === "deposit" && styles.tabTextActive]}>Nạp Heo</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.tab, type === "withdraw" && styles.tabActiveWithdraw]} onPress={() => setType("withdraw")}>
        ...
        {type === "withdraw" && cooldownRemainingDays > 0 ? (
          <View style={styles.cooldownContainer}>
            <View style={styles.cooldownCircle}>
              <Text style={styles.cooldownBigNumber}>{cooldownRemainingDays}</Text>
              <Text style={styles.cooldownDaysLabel}>Ngày chờ</Text>
            </View>
            <View style={styles.withdrawNotice}>
              <Info color="#ef4444" size={16} />
              <Text style={styles.withdrawNoticeText}>Bạn chỉ có thể rút tiết kiệm mỗi 7 ngày một lần.</Text>
            </View>
          </View>
        ) : (
          <>
            {type === "withdraw" && (
              <View style={styles.withdrawNotice}>
                <Info color="#ef4444" size={16} />
                <Text style={styles.withdrawNoticeText}>Hạn mức rút tối đa: 500.000 đ / lần.</Text>
              </View>
            )}
            <View style={[styles.amountDisplay, type === "withdraw" ? styles.borderWithdraw : styles.borderDeposit]}>
              <Text style={[styles.amountText, type === "deposit" ? styles.depositText : styles.withdrawText]}>{formatCurrency(amount)}</Text>
              <Text style={styles.currencyLabel}>VNĐ</Text>
            </View>
            <Text style={styles.sectionTitle}>Chọn mệnh giá</Text>
            <Keypad amount={amount} onAddAmount={(v) => setAmount(p => p + v)} onClear={() => setAmount(0)} />
            <TouchableOpacity style={[styles.saveButton, type === "deposit" ? styles.saveDeposit : styles.saveWithdraw]} onPress={executeTransaction}>
              <Text style={styles.saveButtonText}>{type === "deposit" ? "Xác Nhận Nạp" : "Xác Nhận Rút"}</Text>
            </TouchableOpacity>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: "#f59e0b", padding: 24, paddingTop: 60, borderBottomLeftRadius: 24, borderBottomRightRadius: 24 },
  ...
  historyBtn: { padding: 8, backgroundColor: "rgba(255,255,255,0.2)", borderRadius: 12 },
  ...
  targetCard: { backgroundColor: "#ffffff", borderRadius: 16, padding: 20, marginBottom: 24, elevation: 2, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  ...
  viewHistoryBtn: { backgroundColor: "#f1f5f9", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
  viewHistoryText: { fontSize: 12, color: "#64748b", fontWeight: "600" },
  targetDisplayRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  ...
  targetInput: { flex: 1, backgroundColor: "#f8fafc", borderWidth: 1, borderColor: "#cbd5e1", borderRadius: 8, padding: 12, fontSize: 16 },
  saveTargetBtn: { backgroundColor: "#3b82f6", paddingVertical: 12, paddingHorizontal: 16, borderRadius: 8 },
  saveTargetBtnText: { color: "#fff", fontWeight: "bold" },
  cancelBtn: { backgroundColor: "#ef4444", paddingVertical: 12, paddingHorizontal: 16, borderRadius: 8 },
  cancelBtnText: { color: "#fff", fontWeight: "bold" },
  progressContainer: { height: 8, backgroundColor: "#e2e8f0", borderRadius: 4, overflow: "hidden", marginTop: 8, marginBottom: 8 },
  progressBar: { height: "100%", backgroundColor: "#10b981" },
  progressText: { fontSize: 13, color: "#64748b" },
  cooldownText: { fontSize: 12, color: "#ef4444", marginTop: 8, fontStyle: "italic" },
  ...
  withdrawNotice: { flexDirection: "row", backgroundColor: "#fef2f2", padding: 12, borderRadius: 12, marginBottom: 20, alignItems: "center", gap: 8, borderWidth: 1, borderColor: "#fee2e2" },
  withdrawNoticeText: { fontSize: 13, color: "#b91c1c", flex: 1, lineHeight: 18, fontWeight: "500" },
  amountDisplay: { backgroundColor: "#ffffff", borderRadius: 16, padding: 24, alignItems: "center", flexDirection: "row", justifyContent: "center", marginBottom: 24, borderWidth: 1 },
  ...
  cooldownCircle: { width: 140, height: 140, borderRadius: 70, borderWidth: 8, borderColor: "#ef4444", justifyContent: "center", alignItems: "center", marginBottom: 20 },
