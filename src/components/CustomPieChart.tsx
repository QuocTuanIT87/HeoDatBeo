...
const CustomPieChart: React.FC<Props> = ({ data, selectedCategory, onSelectCategory, renderNoteDetails }) => {
  ...
  data.forEach(item => {
    ...
    if (!groupedData[groupKey]) {
      groupedData[groupKey] = { key: groupKey, population: 0, color: isOther ? '#64748b' : item.color, names: [] };
    }
  ...
  const getCoordinatesForAngle = (angle: number, rad: number) => {
    ...
    return { x: centerX + rad * Math.cos(rad2), y: centerY + rad * Math.sin(rad2) };
  };
  ...
  const arcs = chartSlices.map(slice => {
    ...
    // showLabel sẽ được ghi đè trong bước phân bổ bên dưới khi hideChartLabels
    const showLabel = !hideChartLabels;
    ...
    const lineEnd = { x: lineBreak.x + (isRightSide ? 30 : -30), y: lineBreak.y };

    return { slice, pathData, isSelected, showLabel, midAngle, percent, lineStart, lineBreak, lineEnd, isRightSide };
  });

  // --- Phân bổ vị trí label ---
  if (hideChartLabels) {
    // Bước 1: Tắt hết showLabel
    arcs.forEach(a => { a.showLabel = false; });
    ...
    const rightHalf = angleSorted.filter(a => a.midAngle >= 0 && a.midAngle < 180);
    ...
    const leftCnt  = Math.min(wantPerSide + (wantPerSide - rightCnt), leftHalf.length);
    ...
    arcs.filter(a => a.showLabel && a.isRightSide).forEach(arc => {
  ...
  const selectedArc = selectedCategory
    ? arcs.find(a => a.slice.key === selectedCategory || a.slice.names.includes(selectedCategory))
    : null;
    ...
    <View style={styles.container}>
      ...
      {selectedArc ? (
        <View style={styles.infoBox}>
          <View style={[styles.infoColorDot, { backgroundColor: selectedArc.slice.color }]} />
          <View style={styles.infoTextBlock}>
            <Text style={styles.infoName} numberOfLines={1}>{selectedArc.slice.key}</Text>
            <Text style={styles.infoDetail}>
              {selectedArc.percent} · {formatCurrency(selectedArc.slice.population)} đ
            </Text>
          </View>
          <TouchableOpacity onPress={() => onSelectCategory('')} style={styles.infoClose}>
            <Text style={styles.infoCloseText}>×</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <View style={styles.infoBoxPlaceholder}>
          <Text style={styles.infoPlaceholderText}>Nhấn vào biểu đồ để xem chi tiết</Text>
        </View>
      )}
            ...
            <View key={`main-group-${index}`}>
              ...
              >
                ...
                <View style={styles.listItemRight}>
                  {hideChartLabels && (
                    <Text style={[styles.listPercent, isSelfSelected && styles.listTextSelected]}>
                      {percent}
                    </Text>
                  )}
          ...
          return (
            <View key={`other-group-${index}`}>
              ...
              >
                ...
                <View style={styles.listItemRight}>
                  {hideChartLabels && (
                    <Text style={[styles.listPercent, isItemSelected && styles.listTextSelected]}>
                      {percent}
                    </Text>
                  )}
...
const styles = StyleSheet.create({
  container: { width: '100%', flex: 1, alignItems: 'center', paddingBottom: 20 },
  ...
  infoName: { fontSize: 15, fontWeight: '700', color: '#f1f5f9', marginBottom: 2 },
  ...
  listContainer: { paddingHorizontal: 20, marginTop: 10, paddingBottom: 40, gap: 8 },
  listItem: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 12, paddingHorizontal: 16, backgroundColor: '#f1f5f9', borderRadius: 12,
  },
  ...
  listPercent: { fontSize: 13, color: '#64748b', fontWeight: '600', minWidth: 40, textAlign: 'right' },
  ...
  noteDetailsInList: {
    ...
    padding: 12, marginTop: -4, borderWidth: 1, borderColor: '#e2e8f0',
    borderTopWidth: 0, marginBottom: 8,
  },
  otherHeaderContainer: {
    marginTop: 12, marginBottom: 4, paddingHorizontal: 8,
